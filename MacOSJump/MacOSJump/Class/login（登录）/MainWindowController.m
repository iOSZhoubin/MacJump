...
@property (strong,nonatomic) FirstPageWindowController *firstWindowC;
...
- (void)windowDidLoad {
    ...
    self.firstWindowC = [[FirstPageWindowController alloc]initWithWindowNibName:@"FirstPageWindowController"];
...
-(void)loginAction{
    ...
    [AFNHelper macPost:urlStr parameters:parameters success:^(id responseObject) {
        ...
        if([responseObject[@"message"] isEqualToString:@"ok"]){
            ...
            weakself.firstWindowC.ipAddress = self.ipcontent.stringValue;
            
            weakself.firstWindowC.port = self.portcontent.stringValue;
            
            [weakself.firstWindowC.window orderFront:nil];//显示要跳转的窗口
            
            [[weakself.firstWindowC window] center];//显示在屏幕中间
